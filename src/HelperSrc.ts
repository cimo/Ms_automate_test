...
export const writeLog = (tag: string, value: string | Record<string, unknown> | Error): void => {
    if (process.env.MS_AT_DEBUG === "true") {
        if (typeof process !== "undefined" && process.env.MS_AT_PATH_LOG) {
            Fs.appendFile(`${process.env.MS_AT_PATH_LOG}debug.log`, `${tag}: ${value.toString()}\n`, () => {
...
export const NAME = checkEnv("MS_AT_NAME", process.env.MS_AT_NAME);
export const LABEL = checkEnv("MS_AT_LABEL", process.env.MS_AT_LABEL);
...
export const URL_TEST = checkEnv("MS_AT_URL_TEST", process.env.MS_AT_URL_TEST);
...
export const PATH_PUBLIC = checkEnv("MS_AT_PATH_PUBLIC", process.env.MS_AT_PATH_PUBLIC);
...
export const WS_ADRESS = checkEnv("MS_AT_WS_ADDRESS", process.env.MS_AT_WS_ADDRESS);
export const SECRET_KEY = checkEnv("MS_AT_SECRET_KEY", process.env.MS_AT_SECRET_KEY);
